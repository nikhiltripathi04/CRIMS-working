...
router.delete('/:id', checkSiteOwnership, async (req, res) => {
    try {
        ...
        if (adminUser) {
            ...
            // We can't add to site's activity logs since it's deleted,
            // but we could log this to a separate admin activity log if needed
        }
...
router.post('/:id/supplies', checkSiteOwnership, async (req, res) => {
    try {
        ...
        await ActivityLogger.logActivity(
            ...
            `${userDetails.username} added ${addedSupply.quantity} ${addedSupply.unit} of "${addedSupply.itemName}" (pending pricing)`
        );
